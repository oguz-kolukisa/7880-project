...
class Pascal5iReader(torchvision.datasets.vision.VisionDataset):
    ...
    def __getitem__(self, idx):
        ...
        target_np = target_np.astype(np.uint8, copy=False)
        target_img = Image.fromarray(target_np)
        target_img = target_img.resize((w, h), Image.NEAREST)
        target_np = np.array(target_img, dtype=np.int64)
        
        img = self.to_tensor_func(img).clone()
        target = torch.as_tensor(target_np.copy(), dtype=torch.long)
        return img, target
